...
function UtilizePreviewGrid() {
      const { layeredAssets, collectionInfo, assetMetadata, setAssetMetadata, } = useContext(GenerateLayerContext);
            ...
            <Box>
                  <Stack direction='row' flexWrap='wrap' justifyContent='space-evenly'>
                        {assetMetadata.map((metadata, i) =>
                              <GeneratedAsset data={metadata.attributes.map(attr => attr.asset.data)} width={200} height={200} label={metadata.assetName} key={i} />
                        )}
                  </Stack>
            </Box>
      )
